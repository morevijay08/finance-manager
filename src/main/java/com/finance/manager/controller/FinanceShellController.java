...
public class FinanceShellController extends BudgetDashboardController {
    ...
    private void addSettingsToProfileMenu() {
        ...
        Button settingsButton = new Button("⚙  Settings"); settingsButton.setMaxWidth(Double.MAX_VALUE); settingsButton.getStyleClass().add("profile-settings"); settingsButton.setOnAction(this::handleSettings);
        int logoutIndex = -1;
    ...
    private void installDashboardButtonHandlers(Pane parent) {
        for (Node node : parent.getChildren()) {
            if (node instanceof Button button) {
                String text = button.getText(); if (text == null) continue;
                if (text.contains("Add Transaction")) button.addEventFilter(ActionEvent.ACTION, e -> { e.consume(); activate(null); showSection("addTransactionSection"); });
                else if (text.contains("Recent Activity") || text.contains("View all")) button.addEventFilter(ActionEvent.ACTION, e -> { e.consume(); activate(transactionsNav); showSection("transactionsSection"); });
                else if (text.contains("Analytics")) button.addEventFilter(ActionEvent.ACTION, e -> { e.consume(); activate(analyticsNav); showSection("analyticsSection"); });
                else if (text.contains("Reports")) button.addEventFilter(ActionEvent.ACTION, e -> { e.consume(); activate(reportsNav); showSection("reportsSection"); });
                else if (text.contains("Goals")) button.addEventFilter(ActionEvent.ACTION, e -> { e.consume(); activate(goalsNav); showSection("goalsSection"); });
                else if (text.contains("Alerts")) button.addEventFilter(ActionEvent.ACTION, e -> { e.consume(); activate(notificationsNav); showSection("notificationsSection"); });
                else if (text.contains("Budget")) button.addEventFilter(ActionEvent.ACTION, e -> { e.consume(); activate(budgetNav); showSection("budgetSection"); });
    ...
    private void showSection(String selectedField) {
        ...
        for (String name : sections) { Node section = getSection(name); if (section != null) { boolean selected = name.equals(selectedField); section.setVisible(selected); section.setManaged(selected); section.setMouseTransparent(!selected); } }
        Node selected = getSection(selectedField); if (selected != null) selected.toFront();
    }

    private Node getSection(String fieldName) {
        try { Field field = BudgetDashboardController.class.getDeclaredField(fieldName); field.setAccessible(true); return (Node) field.get(this); }
    ...
    @FXML private void handleSettings(ActionEvent event) {
        closeProfile();
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/fxml/Settings.fxml")); Parent settingsRoot = loader.load();
            ...
            Stage owner = (Stage) ((Node) event.getSource()).getScene().getWindow(); settingsStage.initOwner(owner); settingsStage.setScene(new Scene(settingsRoot, 620, 520)); settingsStage.setResizable(false); settingsStage.showAndWait();
    ...
    private void invokeDashboardActionWithEvent(String methodName, ActionEvent event) { try { Method method = DashboardController.class.getDeclaredMethod(methodName, ActionEvent.class); method.setAccessible(true); method.invoke(this, event); } catch (Exception e) { throw new RuntimeException("Could not execute action: " + methodName, e); } }
